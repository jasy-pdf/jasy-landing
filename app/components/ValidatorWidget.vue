...
type Report = {
  ...
  veraPdf: {
    ...
    profile?: string;
    failedRules?: number;
    failures?: { clause: string; failedChecks: number }[];
  } | null;
};
...
async function validateSample() {
  try {
    ...
    errorMsg.value = "Couldn't load the sample invoice. Please try again.";
    state.value = "error";
  }
}
...
<template>
  ...
  >
    ...
    <span
      class="pointer-events-none absolute left-3 top-3 size-2.5 border-l-2 border-t-2 border-brand-300 dark:border-brand-400/50"
      aria-hidden="true"
    />
    <span
      ...
      >
        ...
        <div class="pointer-events-none flex flex-col items-center gap-3">
          <UIcon
            ...
            :class="
              dragging ? 'text-brand-600 dark:text-brand-200' : 'text-brand-500 dark:text-brand-300'
            "
          />
          <div>
    ...
    <div
      ...
      class="grid gap-5 p-5 sm:p-8 md:grid-cols-[1fr_auto] md:items-center"
    >
      <div class="flex items-center gap-4 overflow-hidden">
        <div
          class="grid size-12 shrink-0 place-items-center rounded-lg bg-brand-100 text-brand-700 dark:bg-white/10 dark:text-brand-200"
        >
      ...
      <ul class="mt-5 space-y-3">
        <li v-for="(step, i) in STEPS" :key="step" class="flex items-center gap-3">
          <span class="grid size-6 place-items-center">
            ...
            <UIcon
              v-else-if="i === stepIndex"
              name="i-lucide-loader-circle"
              class="size-5 animate-spin text-brand-500"
            />
          ...
          <span
            ...
            :class="
              i <= stepIndex
                ...
                : 'text-brand-900/40 dark:text-white/35'
            "
          >
            {{ step }}
          </span>
    ...
    <div v-else-if="state === 'done' && report" class="p-5 sm:p-8">
      ...
      <div
        ...
        :class="{
          ...
          'border-brand-300 bg-brand-50/70 dark:border-white/15 dark:bg-white/5':
      ...
      >
        ...
        <div class="min-w-0">
          ...
          >
            ...
            <template v-else-if="verdict === 'invalid'"
              >Not valid · {{ issueCount }} issue{{ issueCount === 1 ? "" : "s" }}</template
            >
      ...
      <div v-if="verdict !== 'unrecognized'" class="mt-5 space-y-3">
        <!-- EN 16931 -->
        <section
          v-if="report.businessRules"
          class="rounded-lg border border-brand-200/70 p-4 dark:border-white/10"
        >
          <header class="flex items-center justify-between gap-3">
            <span class="font-medium text-brand-900 dark:text-white"
              >{{ report.businessRules.kind }} business rules</span
            >
            <span
              class="inline-flex items-center gap-1.5 text-sm font-medium"
              :class="report.businessRules.valid ? 'text-emerald-600' : 'text-red-500'"
            >
              <UIcon
                :name="report.businessRules.valid ? 'i-lucide-check' : 'i-lucide-x'"
                class="size-4"
              />
              {{
                report.businessRules.valid
                  ? "valid"
                  : `${report.businessRules.errors.length} error${report.businessRules.errors.length === 1 ? "" : "s"}`
              }}
            </span>
          </header>
          <ul v-if="report.businessRules.errors.length" class="mt-3 space-y-1.5">
            <li
              v-for="(e, i) in report.businessRules.errors"
              :key="i"
              class="flex gap-2 text-sm text-brand-900/75 dark:text-white/70"
            >
              <UIcon name="i-lucide-x" class="mt-0.5 size-4 shrink-0 text-red-500" />
              <span
                ><code v-if="e.id" class="font-mono text-xs text-brand-600 dark:text-brand-300"
                  >[{{ e.id }}]</code
                >
                {{ e.text }}</span
              >
        ...
        >
          <header class="flex items-center justify-between gap-3">
            ...
            <span
              class="inline-flex items-center gap-1.5 text-sm font-medium"
              :class="report.pdfA3.valid ? 'text-emerald-600' : 'text-red-500'"
            >
          ...
          <ul v-if="showAllPdfa" class="mt-2 grid gap-1.5 sm:grid-cols-2">
            <li
              v-for="c in report.pdfA3.checks"
              :key="c.id"
              class="flex items-center gap-2 text-sm"
              :class="c.ok ? 'text-brand-900/70 dark:text-white/65' : 'text-red-500'"
            >
              <UIcon
                :name="c.ok ? 'i-lucide-check' : 'i-lucide-x'"
                class="size-4 shrink-0"
                :class="c.ok ? 'text-emerald-600' : 'text-red-500'"
              />
              {{ c.label }}
            </li>
          </ul>
        ...
        >
          <header class="flex items-center justify-between gap-3">
            ...
            <span
              v-if="report.veraPdf.available"
              class="inline-flex items-center gap-1.5 text-sm font-medium"
              :class="report.veraPdf.valid ? 'text-emerald-600' : 'text-red-500'"
            >
              <UIcon
                :name="report.veraPdf.valid ? 'i-lucide-check' : 'i-lucide-x'"
                class="size-4"
              />
              {{
                report.veraPdf.valid
                  ? "compliant"
                  : `${report.veraPdf.failedRules ?? report.veraPdf.failures?.length ?? 0} failed`
              }}
            </span>
            <span v-else class="spec-label text-brand-400">not run</span>
          </header>
          <p
            v-if="report.veraPdf.available && report.veraPdf.profile"
            class="spec-label mt-1.5 text-brand-500 dark:text-brand-300"
          >
            {{ report.veraPdf.profile }}
          </p>
          <ul v-if="report.veraPdf.failures?.length" class="mt-3 space-y-1.5">
            <li
              v-for="(f, i) in report.veraPdf.failures"
              :key="i"
              class="flex gap-2 text-sm text-brand-900/75 dark:text-white/70"
            >
              <UIcon name="i-lucide-x" class="mt-0.5 size-4 shrink-0 text-red-500" />
              <span
                >ISO clause {{ f.clause }}
                <span class="text-brand-400">({{ f.failedChecks }} checks)</span></span
              >
            </li>
          </ul>
        </section>
        ...
        <p class="text-xs leading-relaxed text-brand-900/55 dark:text-white/50">
          Validated with veraPDF, the PDF Association's official open-source PDF/A validator, and
          the EN 16931 (KoSIT) business rules. jasy only runs them - the verdict is theirs.
        </p>
      </div>
