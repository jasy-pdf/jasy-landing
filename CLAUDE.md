...
`@jasy/pdf`@1.0.0-alpha.7 · `@jasy/vue`@1.0.0-alpha.7 · `@jasy/zugferd`@1.0.0-alpha.4 ·
`@jasy/nuxt`@1.0.0-alpha.6 · `@jasy/cli`@1.0.0-alpha.6 (2026-07-23). The engine renders **in the browser
as well as in Node**. Shipped since launch: custom fonts, encryption (AES-256), PDF/UA accessibility,
native colour emoji, relative sizing, rotate, hyperlinks / anchors / bookmarks, page numbers, text
decoration (underline / strikethrough) + letter-spacing, **kerning (on by default)**, and **page-break
control** (`PageBreak`, `breakBefore` / `breakAfter`, `keepTogether`). Still on the roadmap: gradients,
SVG, forms, orphans/widows, and reading/editing existing PDFs. Be honest in copy — don't promise roadmap
items as shipped.
