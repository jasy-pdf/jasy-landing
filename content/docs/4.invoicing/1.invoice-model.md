...
**And a few more:** `delivery` (`date`, `recipientName` and a deliver-to `address` - goods going
somewhere other than the buyer's own address), `payeeName` (when payment goes to someone other than
the seller), `paidAmount` (already paid, subtracted to give the amount due) and `type` (`380` a
commercial invoice, `381` a credit note).
