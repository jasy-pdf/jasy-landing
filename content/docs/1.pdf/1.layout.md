...
Row({ wrap: true, gap: 8, alignContent: "between" }, [
  ...
  Box({ flexShrink: 1 }, [Text("gives way when space runs short")]),
  Box({ flexBasis: 120 }, [Text("starts at 120pt")]),
  Box({ alignSelf: "end" }, [Text("overrides the row's align")]),
]);
```

`flexShrink` defaults to **0**, where CSS says 1. That is deliberate: turning it on by default would
move every layout that already exists. A percentage width inside a flex line resolves against the line
**minus its gaps**, so N columns at `(100/N)%` fit exactly - CSS makes you subtract the gaps yourself.
