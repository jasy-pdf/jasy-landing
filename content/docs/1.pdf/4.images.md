...
Hand it an **SVG** and it stays a vector instead of becoming a bitmap - same call, no flag:

```ts
Image("logo.svg", { width: 120 });
```

See [SVG and Canvas](/docs/pdf/drawing) for what that path covers.
